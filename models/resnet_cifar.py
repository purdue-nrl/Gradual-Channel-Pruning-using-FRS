...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10):
        ...
        self.fc = MaskedLinear(64 * block.expansion, num_classes)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            shortcut = nn.Sequential(
                MaskedConv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False),
...
def resnet56_cifar(num_classes, **kwargs):
    model = ResNet_Cifar(BasicBlock, [9, 9, 9], num_classes=num_classes, **kwargs)
    return model


def resnet110_cifar(num_classes, **kwargs):
    model = ResNet_Cifar(BasicBlock, [18, 18, 18], num_classes=num_classes, **kwargs)
    return model


def resnet164_cifar(num_classes, **kwargs):
    model = ResNet_Cifar(Bottleneck, [18, 18, 18], num_classes=num_classes, **kwargs)
    return model
